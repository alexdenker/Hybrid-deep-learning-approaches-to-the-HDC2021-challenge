...
class UNet(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, channels, skip_channels, kernel_size = 7, 
        ...
        self.inc = InBlock(in_ch, channels[0], use_norm=use_norm, kernel_size=self.kernel_size[0], num_groups=self.num_groups)
...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, skip_ch=4, kernel_size=3, use_norm=True,num_groups=1):
        ...
        if use_norm:
            self.conv = nn.Sequential(
                nn.GroupNorm(num_groups=num_groups, num_channels=in_ch + skip_ch),
        ...
        if self.skip:
            if use_norm:
                self.skip_conv = nn.Sequential(
                    ...
                    nn.GroupNorm(num_groups=num_groups, num_channels=skip_ch),
                    nn.LeakyReLU(0.2, inplace=True))
            else:
                self.skip_conv = nn.Sequential(
                    nn.Conv2d(out_ch, skip_ch, kernel_size=1, stride=1),
